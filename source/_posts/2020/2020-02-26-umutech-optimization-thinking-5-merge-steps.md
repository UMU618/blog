...
在《[优化思维【3】消除没必要步骤](/2019/07/16/umutech-optimization-thinking-3-remove-unnecessary-steps/)》提到一个对象转化的例子：A 对象要转为 B 对象，实现时先把 A 对象转为中间对象 T，再将 T 转为 B 对象，由于两步都很容易实现，一个现有函数即可，所以很多人可能会采用这个思路。这里先转为 T 对象这步其实是没用必要的。
